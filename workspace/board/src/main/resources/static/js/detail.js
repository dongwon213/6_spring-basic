...
// 타임리프는 서버사이드 템플렛 엔진이라서 서버에서 HTML 에 헨더링 할 때 사용
// ajax는 클라리언트 측에서 실행하는 친구이기 떄문에 th 문법은 사용할 수 없다
...
// 페이지가 처음 로드될 떄 댓글 목록 조회 함수가 실행되도록 한다
...
function getComments(boardId){
    $.ajax({
        ...
        success : function(data){
            ...
            // 댓글이 삭제될 해당 세션 비우기 - 이 함수가 호출때마다 새롭게 비우고 sql로 호출
            ...
            data.forEach(function(comment){
                ...
                // 현재 로그인된 계정과 갯글 작성자가 동일하다면 만들어줄 버튼
...
function editComment(commentId){
    ...
    // 데이터를 뿌릴 때는 text, 입력한 데이터를 가져올 떄는 value.
